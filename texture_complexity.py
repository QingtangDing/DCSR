...
def main():
    ...
    if mode == 'pair':
        # cut training data
        GT_folder = r'E:\QingtangDing\dataset\Flickr2K\Flickr2K_train_HR'  # fix to your path
        LR_folder = r'E:\QingtangDing\dataset\Flickr2K\Flickr2K_LR_bicubic\X2'  # fix to your path
        save_GT_folder = r'E:\QingtangDing\dataset\Flickr2K\train_HR_50'
        save_LR_folder = r'E:\QingtangDing\dataset\Flickr2K\train_LR_full\X2'
...
def extract_signle(opt):
    ...
    for path in img_list:
        img_grad += pool.apply_async(worker, args=(path, opt, grad_thres), callback=update).get()
    pool.close()
    pool.join()
    f = open(str(opt['crop_sz']) + '_flickr2k.pkl', "wb")
    pickle.dump(img_grad, f)
    f.close()
